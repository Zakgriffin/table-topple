// Validates sphere-lab.config.json against the same schema the page uses, from
...
// (v23+), and src/sphereLab/configSchema.ts is deliberately pure -- no fetch,
...
import { SphereLabConfigSchema } from '../src/sphereLab/configSchema.ts';

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const CONFIG_PATH = path.join(ROOT, 'sphere-lab.config.json');
