...
// Intrinsics from the most recent pose, held so the IMU can drive the camera
// between reconstructions without them. Only position and orientation are
...
function arRenderLoop() {
  ...
  // with the prediction working perfectly.
  //
  // predictAt returning null means the coast limit was passed -- the overlay
  // is HIDDEN rather than frozen at a stale pose, because a frozen overlay
  // silently claims to still know where the camera is. Disappearing is the
  // honest failure.
...
// file -- so a direct call here would touch them in their temporal dead zone
// and throw at module load. It does not today only because `arOverlayEnabled`
// starts false and the early return fires first, which is an accident of an
// unrelated default rather than a reason. Deferring to the first frame lets
// module evaluation finish, and costs one frame nobody can see (the overlay
// is off at load anyway).
requestAnimationFrame(arRenderLoop);
...
const lastAcceptedQuat = new THREE.Quaternion();
// Rolling tally so the toggle can be judged rather than believed.
let imuStats = { fixes: 0, rejected: 0, predicted: 0, lastVisionDeltaDeg: 0, lastGyroPathDeg: 0 };
...
const framePairs: RotationPair[] = [];
const FRAME_PAIR_CAP = 400;
...
function rotationVectorBetween(a: THREE.Quaternion, b: THREE.Quaternion): THREE.Vector3 {
...
let upDevice: THREE.Vector3 | null = null;
let upCamera: THREE.Vector3 | null = null;
let gravitySign = 1;
const WORLD_UP = new THREE.Vector3(0, 1, 0);
const upSnaps: Array<{ dev: THREE.Vector3; cam: THREE.Vector3; label: string }> = [];
...
function refreshImuFrameContext() {
  ...
  try {
    ...
    if (pd) {
      ...
      if (res.accepted) {
        ...
        upCamera = WORLD_UP.clone().applyQuaternion(pd.recoveredCamQuat.clone().invert()).normalize();
      }
    }

    // Intrinsics are published on EVERY reconstruction, decode or not: they
    // come from the capture's own aspect/FOV rather than from the pose, so a
    ...
    // milliseconds later. With it OFF this is the ONLY writer, and a failed
