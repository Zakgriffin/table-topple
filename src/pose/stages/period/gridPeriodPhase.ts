...
// pose/stages/period/periodSweep.gpu.ts). Everything else in here is pure and synchronous,
...
export async function computeGridPeriodPhase(
  ...
  // `composites` comes from pose/stages/votes/votes.ts's computeGradient2x2Composites
  ...
  // per candidate, see pose/stages/period/periodSweep.gpu.ts); the CPU loop below stays the
