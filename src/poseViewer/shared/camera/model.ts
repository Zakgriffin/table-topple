...
// ── The wire shape for a device-compute pose result ──────────────────────
//
// MOVED HERE from pipeline/capture.ts, which is Pose Viewer SERVER code: this
// type is consumed by `pendingPoseResult` below and by the dev-bridge client,
// so a shared module was reaching into a server one to describe its own field.
// It also cannot live in devBridge/poseResultWire.ts, whose whole property is
// having ZERO imports -- and this references CompositeLine.
//
// Both branches of the receiving end build one of these (the plain-JSON
// no-image branch and the binary poseResultWire.ts branch), so there is exactly
// one declaration of the shape instead of three that could drift apart.
  ...
  debug?: {
    ...
    pipeline?: {
      ...
      lsdRectangles: {
        ...
        accepted: boolean; nfaLog10: number; lineScore: number;
      }[];
    };
  };
