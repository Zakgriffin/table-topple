...
function createHealthBar(): HealthBar {
  ...
  const fillMat = new THREE.MeshBasicMaterial({
    color: HEALTHY, transparent: true, depthTest: false, depthWrite: false,
  });
...
export function updateVitals(d: Denizen, dt: number): boolean {
  ...
  if (t < DEATH_SHAKE_TIME) {
    // A quick shudder, decaying as it goes. Frequency is high and amplitude
    // small and scaled to the figure -- a king staggers further than a
    // soldier, in proportion.
    const decay = 1 - t / DEATH_SHAKE_TIME;
 ...
 *  guards against touching a shared material twice in one pass. */
